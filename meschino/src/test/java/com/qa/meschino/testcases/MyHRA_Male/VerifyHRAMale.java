...
public class VerifyHRAMale extends BaseTest{
	
	
	
	@Test(dataProviderClass=DataProviderFile.class, dataProvider="MyHRALogsMale")
	public void MyHRALog_Male(Hashtable<String, String> data) throws InterruptedException, IOException{
		...
		if(data.get(h.Q38).equalsIgnoreCase("Currently smoking")){
		 ...
		 Thread.sleep(2000);
		 h.clickConfirm();
		 String actual = h.confirmReport();
		 if(!actual.equalsIgnoreCase(data.get("Expected"))){
			 reportFailure("HRA report not generated");
		 }
